...
`
    ...
    if (!parsedResponse.items || !Array.isArray(parsedResponse.items)) {
      console.error("[v0] Invalid response format - items not found or not an array")
      return NextResponse.json({ message: "Invalid response format from AI" }, { status: 500 })
    }

    console.log("[v0] Returning", parsedResponse.items.length, "medicines")
    return NextResponse.json(parsedResponse)
  } catch (error) {
    console.error("[v0] API Error:", error)
    const message = error instanceof Error ? error.message : "Internal server error"
    return NextResponse.json({ message: "Failed to analyze prescription", error: message }, { status: 500 })
  }
}
